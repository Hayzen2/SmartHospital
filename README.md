...
- Seeded doctors: generated by `DataInitializer` using the pattern `doctor{departmentSlug}{docIndex}@hospital.com` with default password `Doctor123@`.
  - `departmentSlug`: department name lowercased with non-alphanumeric characters removed (e.g., `Pediatrics` → `pediatrics`, `Infectious Diseases` → `infectiousdiseases`).
  - `docIndex`: two-digit doctor index within the department (01–05).
  - Examples: `doctorpediatrics01@hospital.com` (first doctor in Pediatrics), `doctorcardiology03@hospital.com` (third doctor in Cardiology).
  - Seeded doctors are assigned `workingHours` = `07:00-12:00,13:00-17:00` (lunch 12:00–13:00) and `availabilityStatus` = `AVAILABLE`.
